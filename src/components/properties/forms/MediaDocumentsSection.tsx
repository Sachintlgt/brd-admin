...
const DocumentItemRow = memo(({ item, index, onUpdate, onRemove, isSubmitting }: DocumentItemRowProps) => (
...
export default function MediaDocumentsSection({
  ...
  const removeDocumentItem = useCallback((index: number) => {
    setDocumentItems(prev => {
  ...
  /* --------------------------------------------------------------------- */
  const ExistingItem = ({ item, onRemove, type }: { item: any; onRemove: (id: string) => void; type: string }) => (
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-500">Max 10 • PDF, DOC, DOCX, XLS, XLSX, Images</span>
          </div>
          ...
          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-xs text-green-800">
              <strong>Tip:</strong> Upload document files first, then enter the document names below for each one.
            </p>
          </div>
        </div>
